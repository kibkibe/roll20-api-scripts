...
1. 캐릭터마다 하나씩 카드 덱을 생성합니다.
2. 이 때 덱의 이름은 **저널에 있는 캐릭터의 이름**과 동일하게 설정합니다.
3. 각 캐릭터 카드 덱의 뒷면을 그 캐릭터가 사용할 기본 표정 이미지로 등록합니다.
4. 카드 덱에 그 캐릭터가 사용할 감정표현의 개수만큼 카드를 추가합니다.
5. 각 카드에 스탠딩 이미지를 삽입하고 카드의 이름을 **감정표현의 이름**으로 설정합니다.
##### option. 엑스트라 캐릭터의 스탠딩 세팅
1. 엑스트라 캐릭터에게도 스탠딩을 사용한다면 [옵션](#옵션)의 `extra_name`에서 설정한 이름의 카드 덱을 생성합니다.
2. 
