...
#### Object Layer
- 배치하는 토큰
  - `vd_panel`: (필수) 대사 텍스트의 밑에 깔릴 배경이 됩니다. 또한 /desc 대사가 표시되는 위치의 기준이 됩니다.
...
![](https://github.com/kibkibe/roll20-api-scripts/blob/master/wiki_image/vd/standing_3.png)
##### 그림5) 캐릭터와 엑스트라의 스탠딩 이미지 덱 구성
...
4. 캐릭터의 기본 표정 이미지인 경우 카드의 이름을 **저널에 등록된 캐릭터의 이름**으로 설정합니다.
5. 캐릭터의 추가 표정 이미지인 경우 카드의 이름을 **캐릭터이름-감정이름**으로 설정합니다.
6. 덱의 카드 뒷면은 기본 엑스트라 캐릭터에 사용할 기본 스탠딩 이미지를 등록합니다. 각 카드들의 개별 뒷면은 설정하지 않으셔도 됩니다.
