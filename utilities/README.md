...
5. 플레이어가 새 세션방에 캐릭터를 등록할 때, 아바타 이미지를 매번 새로 업로드하는 대신에 본인의 라이브러리에 올라가 있는 기존 이미지를 바로 가져와서 적용해주는 스크립트입니다.  
   (명령어: 채팅창의 As를 이미지를 변경할 캐릭터로 변경한 뒤 `!avs 이미지주소`의 형식으로 입력)
