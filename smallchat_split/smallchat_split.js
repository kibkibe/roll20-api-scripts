...
/* (smallchat_split.js) 210128 코드 시작 */
on('ready', function() {
...
if (msg.type == "api"){
    if (msg.content.indexOf("! ") === 0) {
        ...
        let ho = findObjs({ _type: 'handout', name:logname});
        ...
        if (bg) {
            let width = bg.get('width') - margin * 2;
            if (box.length > 0) {
                box = box[0];
            ...
            let amount = Math.ceil(width/font_size/letterspacing*2);
            let idx = 0;
            let length = 0;
            let halfchar = [' ',',','.','\'','"','[',']','(',')','*','^','!','-','~',':',';','<','>','+','l','i','1'];
        ...
        let d = new Date();
        let tz = d.getTime() + (d.getTimezoneOffset() * 60000) + (timezone * 3600000);
        d.setTime(tz);
        ...
        if (!bg) {
            let oa = findObjs({ _type: 'handout', name:onair_name});
            if (oa.length > 0) {
                oa = oa[0];
            } else {
...
/* (smallchat_split.js) 210128 코드 종료 */
