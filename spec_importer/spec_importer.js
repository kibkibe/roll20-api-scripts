	...
	try {
		if (obj.get('current')) {
			...
			is_setting.db_list.forEach(item => {
				...
				if (split.length != 2) {
					sendChat("error","/w gm is_setting의 check_list에 지정된 name값이 접두어+\*id\*+접미어 형식이 아닙니다.",null,{noarchive:true});
					return;
			...
			if (target) {
				...
				dataset[0].get('notes',function(note){
					let data = note.replace(/\<\/p\>\<p\>/g,'\n').replace(/(<([^>]+)>)/gi, '').split('========');
					const magic_name = obj.get('current').replace(/ /g,'');
					for (let index = 0; index < data.length; index++) {
						...
						if (item[0].replace(/ /g,'') == magic_name) {
							for (let j = 0; j < target.output_attrs.length; j++) {
								const attr = target.output_attrs[j];
								let condition = {type:'attribute',name:attr.replace('*id*',id),characterid:obj.get('_characterid')};
